...
object Kit {
    ...
    @Throws(Exception::class)
    fun postRequest(url: String, pairList: ArrayList<BasicNameValuePair>, proxyIP: String, proxyPort: Int): CloseableHttpResponse {
        val httpClient = HttpClients.createDefault()
        val proxy = HttpHost(proxyIP, proxyPort, "http")
...
//        httpClient.close()
    ...
    fun postContent(url: String, pairList: ArrayList<BasicNameValuePair>, proxyIP: String, proxyPort: Int): String {
        val httpClient = HttpClients.createDefault()
        val proxy = HttpHost(proxyIP, proxyPort, "http")
        val requestConfig = RequestConfig.custom().setProxy(proxy).build()
        val httpPost = HttpPost(url)
